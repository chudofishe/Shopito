...
class SharedPrefAppStore(private val appContext: Context) {
    ...
    companion object {
        ...
        const val SHARED_PREF_WATCHED_ONBOARDING_KEY = "ACTIVE_LIST_STATE"
    }

    fun saveWatchedOnBoarding() {
        val sharedPreferences =
            appContext.getSharedPreferences(SharedPrefDataStore.SHARED_PREF_FILE_KEY, Context.MODE_PRIVATE).edit()
    ...
    fun getWatchedOnBoarding(): Boolean {
        val sharedPreferences =
            appContext.getSharedPreferences(SharedPrefDataStore.SHARED_PREF_FILE_KEY, Context.MODE_PRIVATE)
